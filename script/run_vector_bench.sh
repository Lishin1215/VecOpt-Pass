...
for i in $(seq 1 $REPS); do
  SEC=$({ /usr/bin/time -p ./qsort_baseline "$QSORT_INPUT" 1>/dev/null; } 2>&1 | awk '/^real/ {print $2}')
  echo "qsort,baseline,$i,$SEC" >> "$QSORT_CSV"
done
for i in $(seq 1 $REPS); do
  SEC=$({ /usr/bin/time -p ./qsort_vecopt "$QSORT_INPUT" 1>/dev/null; } 2>&1 | awk '/^real/ {print $2}')
  echo "qsort,vecopt,$i,$SEC" >> "$QSORT_CSV"
done

popd >/dev/null

QSORT_BASELINE_AVG=$(awk -F, '$1=="qsort" && $2=="baseline"{s+=$4;n++} END{printf "%.4f", s/n}' "$QSORT_CSV")
QSORT_VECOPT_AVG=$(awk -F, '$1=="qsort" && $2=="vecopt"{s+=$4;n++} END{printf "%.4f", s/n}' "$QSORT_CSV")
...
REPS=50
for i in $(seq 1 $REPS); do
  SEC=$({ /usr/bin/time -p ./basicmath_baseline 1>/dev/null; } 2>&1 | awk '/^real/ {print $2}')
  echo "basicmath,baseline,$i,$SEC" >> "$BASICMATH_CSV"
done
for i in $(seq 1 $REPS); do
  SEC=$({ /usr/bin/time -p ./basicmath_vecopt 1>/dev/null; } 2>&1 | awk '/^real/ {print $2}')
  echo "basicmath,vecopt,$i,$SEC" >> "$BASICMATH_CSV"
done

popd >/dev/null

BASICMATH_BASELINE_AVG=$(awk -F, '$1=="basicmath" && $2=="baseline"{s+=$4;n++} END{printf "%.4f", s/n}' "$BASICMATH_CSV")
BASICMATH_VECOPT_AVG=$(awk -F, '$1=="basicmath" && $2=="vecopt"{s+=$4;n++} END{printf "%.4f", s/n}' "$BASICMATH_CSV")
...
REPS=5
for i in $(seq 1 $REPS); do
  SEC=$({ /usr/bin/time -p ./blackscholes_baseline 1 "$BLACKSCH_INPUT" "$BLACKSCH_OUTPUT" 1>/dev/null; } 2>&1 | awk '/^real/ {print $2}')
  echo "blackscholes,baseline,$i,$SEC" >> "$BLACKSCH_CSV"
done
for i in $(seq 1 $REPS); do
  SEC=$({ /usr/bin/time -p ./blackscholes_vecopt 1 "$BLACKSCH_INPUT" "$BLACKSCH_OUTPUT" 1>/dev/null; } 2>&1 | awk '/^real/ {print $2}')
  echo "blackscholes,vecopt,$i,$SEC" >> "$BLACKSCH_CSV"
done

popd >/dev/null

BLACKSCH_BASELINE_AVG=$(awk -F, '$1=="blackscholes" && $2=="baseline"{s+=$4;n++} END{printf "%.4f", s/n}' "$BLACKSCH_CSV")
BLACKSCH_VECOPT_AVG=$(awk -F, '$1=="blackscholes" && $2=="vecopt"{s+=$4;n++} END{printf "%.4f", s/n}' "$BLACKSCH_CSV")
